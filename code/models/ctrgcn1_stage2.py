...
class cv_TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x, case, vlist, xr): # or x, case, vlist, xr
        # case=0 means basic, case=1 means cross-view, vlist means view list
        if case == 0:
            y = self.relu(self.tcn1(self.gcn1(x)) + self.residual(x))
        else:
            # CV-FCL (CV-STGCN) or GL-CVFD (only CV-GCN), the second S-FCL or S-CVFD training stage
            # 
            temp1 = x
            temp2 = xr
            # normal GCNs
...
class Model(nn.Module):
    ...
    def forward(self, x, xr): # x (V-FCL) or x, xr (S-FCL)
        N, C, T, V, M = x.size()
        ...
        # 第二阶段 semantic-term training stage, assistant view
        c_new2 = xcv1.size(1)
