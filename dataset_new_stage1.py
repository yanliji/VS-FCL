...
class NTUDataLoaders(object):
    def __init__(self, dataset = 'NTU', case = 1):
        ...
        self.s4 = CustomBatchSampler(self.s3, 64, True) # 128
    ...
    def get_train_loader(self, batch_size, num_workers):
        ...
        # same-action and different-action data loaders for parallel training of V-FCL(V-CVFD) and S-FCL(S-CVFD)
        # return DataLoader(self.train_set, batch_sampler = self.s2, num_workers=num_workers, collate_fn=self.collate_fn, pin_memory=True), DataLoader(self.train_set, batch_size=batch_size, shuffle=True, num_workers=num_workers, collate_fn=self.collate_fn, pin_memory=True)
        
        # The following line is a first-stage training set, with the key emphasis being on the inclusion of a batch_sampler.
        return DataLoader(self.train_set, batch_sampler = self.s2, num_workers=num_workers, collate_fn=self.collate_fn, pin_memory=True)
    ...
    def get_val_loader(self, batch_size, num_workers):
        ...
        return DataLoader(self.val_set, batch_sampler=self.s4, num_workers=num_workers, collate_fn=self.collate_fn,pin_memory=True)
        #return DataLoader(self.val_set, batch_sampler = self.s4, num_workers=num_workers, collate_fn=self.collate_fn, pin_memory=True), DataLoader(self.val_set, batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=self.collate_fn, pin_memory=True)
    ...
    def get_test_loader(self, batch_size, num_workers):
        ...
        return DataLoader(self.test_set, batch_sampler = self.s6, num_workers=num_workers, collate_fn=self.collate_fn, pin_memory=True)
