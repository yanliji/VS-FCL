    ...
    def forward(self, x):
        ...
        if self.use_data_bn:
            if self.M_dim_bn:
                ...
                xv = xv.permute(0, 4, 3, 1, 2).contiguous().view(N * M, V * C, T)
            xv = self.data_bn(xv)
            xv = xv.view(N, M, V, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V)
        else:
...
class STTransformer(nn.Module):
    def __init__(self,
        ...
        #self.mlp3 = nn.Linear(60, 1024, bias=False)
        #self.bn3 = nn.BatchNorm1d(1024)
        #self.relu3 = nn.ReLU(inplace=True)
        #self.mlp4 = nn.Linear(1024, 60, bias=False)
        #self.bnlast = nn.BatchNorm1d(60, affine=False)
        #self.encoder = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.encoder)
        #self.encoder_k = torch.nn.DataParallel(self.encoder)

    def forward(self, x1):
        x = _transform_shape(x1)
        x = self.encoder_s(x)
        return x
...
def _transform_rotation(inputs1):
    ...
    for i in range(x.shape[0]):
        for j in range(x.shape[1]):
            x[i][j] = random.randint(0,math.pi/4)
    y = np.empty([inputs1.shape[0],1], dtype = float) 
    for i in range(y.shape[0]):
        for j in range(y.shape[1]):
            y[i][j] = random.randint(0,math.pi/4)
    z = np.empty([inputs1.shape[0],1], dtype = float) 
    for i in range(z.shape[0]):
        for j in range(z.shape[1]):
            z[i][j] = random.randint(0,math.pi/4)
