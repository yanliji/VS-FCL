...
class tcn_unit_attention(nn.Module):
    def __init__(self, in_channels, out_channels, dv_factor, dk_factor, Nh, n,
                 ...
                 last, layer, device, more_channels, drop_connect, num_point, bn_flag=True,
        ...
        if self.relative:
            if self.more_channels:
                self.key_rel = nn.Parameter(torch.randn((2 * frames - 1, self.dk // self.num), requires_grad=True))

            else:
                self.key_rel = nn.Parameter(torch.randn((2 * frames - 1, self.dk // Nh), requires_grad=True))
