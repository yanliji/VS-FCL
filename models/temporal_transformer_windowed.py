...
class tcn_unit_attention_block(nn.Module):
    def __init__(self, in_channels, out_channels, dv_factor, dk_factor, Nh,
                 ...
                 bn_flag=True, shape=25, visualization=False, data_normalization=True, skip_conn=True, more_relative=False):
        ...
        if ((self.in_channels != self.out_channels) or (stride != 1)):
            self.down = Unit2D(self.in_channels, self.out_channels, kernel_size=1, stride=stride)
        else:
            self.down = None
        ...
        if (not self.only_temporal_att):
            self.tcn_conv = Unit2D(in_channels, out_channels - self.dv, 
                                   dropout=dropout,
        ...
        if self.relative:
            if self.more_channels:
                self.key_rel = nn.Parameter(torch.randn((2 * self.block_dim - 1, self.dk // self.num), requires_grad=True))

            else:
                self.key_rel = nn.Parameter(torch.randn((2 * self.block_dim - 1, self.dk // Nh), requires_grad=True))
